...
class Scanner:
    ...
    def skip_white_space(self):
        ...
        '''
        # While index in the list and current is a space skip it
    ...
    def get_token(self):
        ...
        '''
        # Skips upto the next token
        self.skip_white_space()
        
        token, longest = None, '' # Sets value to none to see if changed
        for (t, r) in Token.token_regexp:
            # Try each token see if it matched the current token
            match = re.match(r, self.input_string[self.current_char_index:])

            if match and match.end() > len(longest):
                # If it is matched then assign it to the values token, longest
                token, longest = t, match.group()
        
        # If the token is found incriment the index by the length of the token (skip over it)
        self.current_char_index += len(longest)

        # Check to see if the input hasn't changed meaning that it could not be matched
        if self.current_char_index < len(self.input_string) and token is None:
            self.no_token()
    ...
    def consume(self, *expected_tokens):
        ...
        '''
        # Getting the token and value
        token, longest = self.current_token

        if token in [Token.NUM, Token.ID]:
            # If is a NUM or ID token
            self.current_token = self.get_token()
            return token, longest
        
        elif token in expected_tokens:
            # If it is any other token
            self.current_token = self.get_token()
            return token
        
        else: 
            # For unknown tokens raise error
            return self.unexpected_token(token, expected_tokens)
...
class If_Else_AST:
    def __init__(self, condition, then, _else_):
        self.condition = condition
        self.then = then
        self._else_ = _else_
    def __repr__(self):
        return 'if ' + repr(self.condition) + ' then ' + \
                       repr(self.then) + ' else ' + \
                       repr(self._else_) + ' end'
    def indented(self, level):
        return indent('If-Else', level) + \
               self.condition.indented(level+1) + \
               self.then.indented(level+1) + \
               self._else_.indented(level+1)
    def code(self):
        l1 = label_generator.next()
...
def if_statement():
    ...
    if scanner.lookahead() == Token.ELSE:
        ...
        return If_Else_AST(condition, then, _else_)
    else:
        scanner.consume(Token.END)
